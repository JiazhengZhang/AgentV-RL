...
class vLLMAgentRollout(BaseRollout):
    ...
    @GPUMemoryLogger(role="vllm tool rollout spmd", logger=logger)
    @torch.no_grad()
    def generate_sequences_tool_only(self, prompts: DataProto, **kwargs) -> DataProto:
        """Generate sequences for a batch of prompts 
        by runing multi-turn tool-augmented inference and generate the response sequences
        ...
        # Active indices for multi-turn inference
        unfinished = list(range(batch_size))
        self.tool_logger.info("Vllm tool engine starting inference")
        with self.update_sampling_params(**sp_overrides):
            for round_index in range(max_rounds):
                if not unfinished:
                    break
                ...
                for st_idx, i in enumerate(unfinished):
                    ...
                    if delta:
                        ...
                        curr_ids[i].extend(delta_ids)
                        resp_ids[i].extend(delta_ids)
                        resp_mask[i].extend([0] * len(delta_ids)) # Mask tool-call ids
                        if lp_history is not None:
                            lp_history[i].extend([-1.0] * len(delta_ids))  # Mask tool-call ids log probs
                    # Update text tails and round cnt
                    text_tails[i] = new_text
    ...
    @torch.no_grad()
    def generate_sequences_reserve(self, prompts: DataProto, **kwargs) -> DataProto:
        """Generate sequences for a batch of prompts. \n
        Since tool rollout needs raw string inputs, we simply decode and encode the \n
        input prompts for conveniency. 
        The inputs should not apply chat template since the tool runner will use chat template inside. \n
        ...
        # Prepaire string-like input tokens for our tool pipeline for conveniency
        # We sacrifice some effeciency for extra encode&decode steps
        ...
        with self.update_sampling_params(**kwargs):
            ...
            # TODO Rewrite tool-pipeline for
        ...
        if self.config.calculate_log_probs:
            # we will recompute old log prob with actor
            batch["rollout_log_probs"] = rollout_log_probs
        print(f"Calling vllm tool rollout")
        # TODO add tool-call dict, difficulty to non_tensor_batch 
        return DataProto(batch=batch, non_tensor_batch=non_tensor_batch)
